...
public class DatabaseModel {
	...
	public boolean connect() {
		try {
			...
			conn = DriverManager.getConnection(
				"jdbc:oracle:thin:@grad.icmc.usp.br:15215:orcl", username,
				password
			);
			...
			if (e.getMessage().contains("onnection")) {
				...
				Logger.log(
					"Failed to login with the user/password combination");
			}
		}
		return false;
	}
	...
	public boolean openResultSet(String sql) {
		...
		try {
			...
			Logger.log(
				"Failed to connect to the database while querying " + sql + "\n err: " + e.getMessage());
		}
		return true;
	}
	...
	public Map<String, String[]> getCheckInConstraint(String tableName) {
		...
		String sql = "SELECT SEARCH_CONDITION FROM user_constraints" +
	...
	public Map<String, String[]> getForeignKeyConstraints(String tableName) {
		...
		doTransaction(sql, rs -> {
			try {
				while (rs.next()) {
					String[] foreign = new String[]{rs.getString(
						3), rs.getString(2)};
...
//		});
		...
		FKs.forEach((local, foreign) -> {
			String eachSQL = "SELECT UNIQUE " +
				foreign[1] + " FROM " + foreign[0] +
				" ORDER BY " + foreign[1];
			ArrayList<String> possible = new ArrayList<>();
			doTransaction(eachSQL, rs -> {
				try {
					while (rs.next()) possible.add(rs.getString(1));
				} catch (SQLException e) {
	...
	public ArrayList<String> getConstraints(
		String tableName, char constraintType
	) {
	...
	public boolean useEachRow(
		String table, String column, LambdaUser<String> user
	) {
		...
		doTransaction(sql, rs -> {
			try {
				while (rs.next()) user.use(
					rs.getString(column.replaceAll("\\s", "").toUpperCase()));
			} catch (SQLException e) {
				Logger.log(
					"Failed to connect to the database while querying " + table + "." + column);
			}
		});
		return true;
	}
	...
	public Map<String, String> getPrivilegesFor(String table) {
		...
		doTransaction(sql, rs -> {
			try {
				while (rs.next()) {
					String newValue = rs.getString("privilege");
					ret.compute(rs.getString("grantee"), (key, oldValue) -> {
	...
	public boolean insertIntoTable(String tableName, Map<String, String> data) {
		...
		try {
			...
			Logger.log("Failed to insert data into database, error: " + e.getMessage());
			return false;
		}
